...
namespace Misc
{ // mostly stuff to mark uninitialised props and some generic tags for nodes and edges alike
...
const std::string sRef2 =
    "reference2"; // References a field on the pointee node as a Reference inside the pointing node (ptr2ptr)
const std::string sAdr =
    "address"; // References the address of a field on the pointee node as a field inside the pointing node

const std::string sDyn[] = { sIm, sAdr, sRef, sRef2 }; // array of dynamic edge types
const std::string sWrite =
    "write"; // Writes a value from a field inside the pointing node to a field inside the pointee node
...
const std::string sSection = "section"; // With globals, we often need to specify, for example "register", currenttime",
                                        // "threaddata", "threadstart"
const std::string sThread =
    "thread"; // For cmd dots, designates target thread. With thread specific globals, it chooses the thread
...
{
// Block Parameters
const std::string sTimePeriod =
    "tperiod"; // a block's period (duration) which is added to thread's cumulative time sum in ns
const std::string sGenQPrioHi =
    "qil"; // order to generate (if none exists) and attach a high priority queue to this block
const std::string sGenQPrioMd =
    "qhi"; // order to generate (if none exists) and attach a medium priority queue to this block
const std::string sGenQPrioLo =
    "qlo"; // order to generate (if none exists) and attach a low priority queue to this block
const std::string sGenQPrio[] = { sGenQPrioLo,
                                  sGenQPrioMd,
                                  sGenQPrioHi }; // array of priority generation orders for ease of use
} // namespace Block
...
const std::string sDst =
    "dest"; // a flow command's destination node. only used in cmd dots, schedules use edges instead
const std::string sDstPattern =
    "destpattern"; // a flow command's destination pattern. only used in cmd dots, schedules use edges instead
const std::string sDstBeamproc =
    "destbeamproc"; // a flow command's destination beam proccess. only used in cmd dots, schedules use edges instead
...
const std::string sSwitch =
    "switch"; // switch command (instantaneously switch defdest of a block. Like permanent flow with no queue )
const std::string sOrigin   = "origin"; // origin
const std::string sCmdFlush = "flush";  // flush command (clears a command queue)
const std::string sCmdWait =
    "wait"; // wait command (relative prolongs block duration, abs waits til given time is reached )
...
const std::string sStartThread =
    "startthread";                           // For schedules. Starts a thread at a given time relative to current tsum
...
const std::string sBlockAlign    = "blockalign"; // auto aligning block, prolongs duration to match time grid (currently
                                                 // hardcoded in FW to 10µs starting at TAI 0)
...
const std::string sGlobal  = "global"; // global nodes. This binds a graph node to a specific memory location, eg. WR
                                       // time or a thread's overhead data
...
const std::string sLookVert =
    "rankdir   = TB, nodesep           = 0.6, mindist     = 1.0, ranksep = 1.0, overlap = false";
const std::string sLookHor =
    "rankdir   = LR, nodesep           = 0.6, mindist     = 1.0, ranksep = 1.0, overlap = false";
} // namespace Graph
