...
void init()
{
  ...
  cRules.insert(
      ConstellationRule( n::sSwitch, e::sSwitchTarget, { n::sBlock, n::sBlockFixed, n::sBlockAlign }, 0, 1 ) );
  ...
  cRules.insert(
      ConstellationRule( n::sCmdFlush, e::sCmdTarget, { n::sBlock, n::sBlockFixed, n::sBlockAlign }, 0, 1 ) );
...
void neighbourhoodCheck( vertex_t v, Graph& g )
{
  ...
  for ( out_cur = out_begin; out_cur != out_end; ++out_cur )
  {
    ...
    if ( itCnt == cCnt.end() )
      throw std::runtime_error(
          exIntro +
          " has malfunctioning constellation counter - possible problem with a boost multiindex container\n" );
    cCnt.modify( itCnt,
                 []( ConstellationCnt& e )
                 {
                   e.cnt++;
  ...
  for ( auto itCntChk : cCnt )
  {
    ...
    for ( auto itPCh : itRules->children )
      possibleChildren += ( itPCh + ", " );

    if ( ( itCntChk.cnt < itRules->min ) | ( itCntChk.cnt > itRules->max ) )
    {
      throw std::runtime_error( exIntro + "' must have between " + std::to_string( itRules->min ) + " and " +
                                std::to_string( itRules->max ) + " edge(s) of type '" + itRules->edge +
                                "'' connected to children of type(s) '" + possibleChildren + "', found " +
                                std::to_string( itCntChk.cnt ) + "\n" );
    }
  }
}
...
void eventSequenceCheck( vertex_t v, Graph& g, bool force )
{
  ...
  while ( infiniteLoopGuard < MaxOccurrance::EVENT )
  {
    ...
    for ( out_cur = out_begin; out_cur != out_end; ++out_cur )
    {
      // std::cout << g[vcurrent].name << " child " << g[target(*out_cur,g)].name << " connected by " <<
      // g[*out_cur].type << std::endl;
    ...
    if ( ( g[vnext].type == n::sBlockFixed ) || ( g[vnext].type == n::sBlockAlign ) )
    { // found a block
      // check for forbidden properties:

      if ( g[vcurrent].np == nullptr )
        throw std::runtime_error( g[vcurrent].name + " is not allocated\n" );
      if ( g[vnext].np == nullptr )
        throw std::runtime_error( g[vnext].name + " is not allocated\n" );
      if ( !force )
      { // this allows negative values for time offsets to provoke specific late events
        ...
        if ( boost::dynamic_pointer_cast<Event>( g[vcurrent].np )->getTOffs() >=
             boost::dynamic_pointer_cast<Block>( g[vnext].np )->getTPeriod() )
    ...
    { // this allows negative values for time offsets to provoke specific late events
      if ( boost::dynamic_pointer_cast<Event>( g[vcurrent].np )->getTOffs() >
           boost::dynamic_pointer_cast<Event>( g[vnext].np )->getTOffs() )
