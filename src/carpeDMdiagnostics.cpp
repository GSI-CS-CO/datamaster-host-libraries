...
bool CarpeDM::CarpeDMimpl::tableCheck( std::string& report )
{
  ...
  std::string intro = "*** Table Status:  ", qtyIntro = "*** Element Count: ", allocIntro = "*** Alloctable:    ",
              groupsIntro = "*** GroupTable:    ", hashIntro = "*** Hashtable:     ";
  ...
  qtyIsOk &= ( ( nodeQty == allocEntryQty ) & ( nodeQty == allocEntryQty ) & ( nodeQty == groupsEntryQty ) &
               ( nodeQty == hashEntryQty ) );
  ...
  {
    // Check Hashtable
    if ( !hm.contains( g[v].name ) )
    {
      hashIsOk = false;
      hashReport += sMiss + sFirst + g[v].name + "\n";
    }
    // Check Alloctable
    try
    {
      ...
      (void)dummy;
      struct dummy; // suppress gcc warning about unused variable
    }
    catch ( ... )
    {
      allocIsOk = false;
      allocReport += sMiss + sFirst + g[v].name + "\n";
    }
    // Check Groupstable
    ...
    {
      groupsIsOk = false;
      groupsReport += sMiss + sFirst + g[v].name + "\n";
    }
  }
  ...
  /*
      ...
      bool notFound;
      for (auto& patternIt : gt.getAllPatterns()) { //NOTE: use the pattern list in GroupTable, not the list in the
     Graph!
...
std::string& CarpeDM::CarpeDMimpl::inspectQueues( const std::string& blockName, std::string& report )
{
  ...
  for ( int8_t prio = PRIO_IL; prio >= PRIO_LO; prio-- )
  {
    ...
    for ( uint8_t i, idx = qr.aQ[prio].rdIdx; idx < qr.aQ[prio].rdIdx + 4; idx++ )
    {
      ...
      auxstream << std::setfill( '0' ) << std::setw( 16 ) << std::hex << qe.validTime << " " << std::setfill( '0' )
                << std::setw( 19 ) << std::dec << qe.validTime;
      ...
      case ACT_TYPE_NOOP:
      {
        report += qe.sType + "    Qty: " + std::to_string( qe.qty ) + "\n";
        break;
      }
      case ACT_TYPE_FLOW:
      {
        report += qe.sType + "    Permanent: " + ( qe.flowPerma ? sYes : sNo ) +
                  "    Qty: " + std::to_string( qe.qty ) + "    " + blockName + " --> " + qe.flowDst + " \n";
        break;
      }
      case ACT_TYPE_FLUSH:
      {
        report += qe.sType + "    Flushing:" + "   2 (" + dnt::sQPrio[2] + "): " + ( qe.flushIl ? sYes : sNo ) +
                  "   1 (" + dnt::sQPrio[1] + "): " + ( qe.flushHi ? sYes : sNo ) + "   0 (" + dnt::sQPrio[0] +
                  "): " + ( qe.flushLo ? sYes : sNo ) + "    Qty: " + std::to_string( qe.qty ) + "    " + blockName +
                  " --> " + qe.flushOvr + " \n";
        break;
      }
      case ACT_TYPE_WAIT:
      {
        report +=
            qe.sType + ( qe.waitAbs ? " - until " : " - make block period " ) + std::to_string( qe.waitTime ) + "ns\n";
        break;
      }
      default:
      {
...
std::string& CarpeDM::CarpeDMimpl::getRawQReport( const std::string& blockName, std::string& report )
{
  ...
  for ( int8_t prio = PRIO_IL; prio >= PRIO_LO; prio-- )
  {
    ...
    report += ",RI:" + std::to_string( (int)qr.aQ[prio].rdIdx ) + ",WI:" + std::to_string( (int)qr.aQ[prio].wrIdx ) +
    ...
    for ( uint8_t i, idx = qr.aQ[prio].rdIdx; idx < qr.aQ[prio].rdIdx + 4; idx++ )
    {
      ...
      report += "@RI:" + std::to_string( i ) + ",PEN:" + ( qe.pending ? std::string( "1" ) : std::string( "0" ) ) +
                ",ORPH:" + ( qe.orphaned ? std::string( "1" ) : std::string( "0" ) );
      report += "\nVABS:" + ( qe.validAbs ? std::string( "1" ) : std::string( "0" ) ) +
                ",VTIME:" + std::to_string( qe.validTime );
      report += "\nTYPE:" + std::to_string( qe.type ) + ",STYPE:" + qe.sType;
      report += "\nQTY:" + std::to_string( qe.qty ) + ",FLOWDST:" + qe.flowDst + ",FLOWDSTPAT:" + qe.flowDstPattern;
      report += "\nPERMA:" + ( qe.flowPerma ? std::string( "1" ) : std::string( "0" ) ) +
                ",FPRIO:" + ( qe.flushIl ? std::string( "1" ) : std::string( "0" ) ) +
                ( qe.flushHi ? std::string( "1" ) : std::string( "0" ) ) +
                ( qe.flushLo ? std::string( "1" ) : std::string( "0" ) );
      report += "\nWABS:" + ( qe.waitAbs ? std::string( "1" ) : std::string( "0" ) ) +
                ",WTIME:" + std::to_string( qe.waitTime );
      report += "\n";
    }
  }
...
QueueReport& CarpeDM::CarpeDMimpl::getQReport( Graph&             g,
                                               AllocTable&        at,
                                               const std::string& blockName,
                                               QueueReport&       qr,
                                               const vStrC&       futureOrphan )
{
  ...
  if ( verbose )
    sLog << "Check for orphaned commands is scanning Queue @ " << blockName << std::endl;
  ...
  for ( uint8_t prio = PRIO_LO; prio <= PRIO_IL; prio++ )
  {
    ...
    try
    {
      auto bufLst = at.lookupAdr(
          x->cpu,
          at.adrConv( bufLstAdrType, AdrType::MGMT, x->cpu, bufLstAdr ) ); // buffer list cpu is the same as block cpu
      qr.hasQ[prio] = true;
      bBL           = bufLst->b;
    }
    catch ( ... )
    ...
    if ( pendingCnt )
    {
      for ( uint8_t pidx = rdIdx; pidx < ( rdIdx + pendingCnt ); pidx++ )
      {
        pendingIdx.insert( pidx & Q_IDX_MAX_MSK );
      }
    }

    if ( verbose )
      sLog << "Prio " << (int)prio << std::endl;
    ...
    for ( uint8_t i = 0; i <= Q_IDX_MAX_MSK; i++ )
    {
      ...
      try
      {
        auto buf = at.lookupAdr(
            x->cpu, at.adrConv( bufAdrType, AdrType::MGMT, x->cpu, bufAdr ) ); // buffer cpu is the same as block cpu
...
QueueElement& CarpeDM::CarpeDMimpl::getQelement( Graph&        g,
                                                 AllocTable&   at,
                                                 uint8_t       idx,
                                                 amI           allocIt,
                                                 QueueElement& qe,
                                                 const vStrC&  futureOrphan )
{
  ...
  case ACT_TYPE_FLOW:
  {
    ...
    if ( dstAdr == LM32_NULL_PTR )
    {
      sDst = DotStr::Node::Special::sIdle;
    } // pointing to idle is always okay
    else
    {
      ...
      try
      {
        ...
        for ( auto& itOrphan : futureOrphan )
        {
          ...
          {
            if ( verbose )
              sLog << "found orphaned command pointing to " << itOrphan << " in slot " << (int)idx << std::endl;
            qe.orphaned = true;
            break;
          }
        }
      ...
      {
        if ( verbose )
          sLog << "found orphaned command pointing to unknown destination (#" << std::dec << (int)dstCpu << " 0x"
               << std::hex << dstAdr << std::dec << " in slot " << (int)idx << std::endl;
...
void CarpeDM::CarpeDMimpl::showPaint()
{
  ...
  {
    if ( !g[v].np->isMeta() )
      std::cout << g[v].name << ":" << (int)( g[v].np->isPainted() ? 1 : 0 ) << std::endl;
  }
}
...
void CarpeDM::CarpeDMimpl::inspectHeap( uint8_t cpuIdx )
{
  ...
  for ( int i = 0; i < ebd.getThrQty(); i++ )
    vRa.push_back( heapAdr + i * _PTR_SIZE_ );
  heap = ebd.readCycle( vRa );

  sLog << std::setfill( ' ' ) << "CPU "
       << "Rank  " << std::setfill( ' ' ) << std::setw( 5 ) << "Thread  " << std::setfill( ' ' ) << std::setw( 21 )
       << "Deadline  " << std::setfill( ' ' ) << std::setw( 21 ) << "Origin  " << std::setfill( ' ' ) << std::setw( 21 )
       << "Cursor" << std::endl;

  for ( int i = 0; i < ebd.getThrQty(); i++ )
  {

    uint8_t thrIdx = ( writeBeBytesToLeNumber<uint32_t>( (uint8_t*)&heap[i * _PTR_SIZE_] ) -
                       atDown.adrConv( AdrType::EXT, AdrType::INT, cpuIdx, thrAdr ) ) /
                     _T_TD_SIZE_;
    sLog << std::dec << std::setfill( ' ' ) << std::setw( 3 ) << (int)cpuIdx << std::setfill( ' ' ) << std::setw( 5 )
         << i << std::setfill( ' ' ) << std::setw( 8 ) << (int)thrIdx << std::setfill( ' ' ) << std::setw( 21 )
         << getThrDeadline( cpuIdx, thrIdx ) << std::setfill( ' ' ) << std::setw( 21 ) << getThrOrigin( cpuIdx, thrIdx )
...
vEbwrs& CarpeDM::CarpeDMimpl::clearHealth( vEbwrs& ew, uint8_t cpuIdx )
{
  ...
  const uint32_t baseAdr = atDown.getMemories()[cpuIdx].extBaseAdr + atDown.getMemories()[cpuIdx].sharedOffs;
...
HealthReport& CarpeDM::CarpeDMimpl::getHealth( uint8_t cpuIdx, HealthReport& hr )
{
  ...
  for ( uint32_t offs = 0; offs < _T_DIAG_SIZE_; offs += _32b_SIZE_ )
    diagAdr.push_back( baseAdr + SHCTL_DIAG + offs );
  ...
  for ( int i = 0; i < 8; i++ )
  { // copy and sanitize issuer name
    char c           = *(char*)( b + T_DIAG_SCH_MOD + T_MOD_INFO_IID + i );
    hr.smodIssuer[i] = ( ( ( c > 32 ) && ( c < 126 ) ) ? c : '\00' );
  }
  hr.smodIssuer[8] = '\00';
  for ( int i = 0; i < 8; i++ )
  { // copy and sanitize issuer machine name
    char c         = *(char*)( b + T_DIAG_SCH_MOD + T_MOD_INFO_MID + i );
    hr.smodHost[i] = ( ( ( c > 32 ) && ( c < 126 ) ) ? c : '\00' );
  }
  hr.smodHost[8] = '\00';

  uint8_t schOpType = writeBeBytesToLeNumber<uint32_t>(
      b + T_DIAG_SCH_MOD +
      T_MOD_INFO_TYPE ); // there may be more info here later, so don't use byte offsets, just mask (by cast now)

  // printf("Schedule Optype 0x%02x @ 0x%08x\n", schOpType, T_DIAG_SCH_MOD + T_MOD_INFO_TYPE);

  switch ( schOpType )
  {
  case OP_TYPE_SCH_CLEAR:
    hr.smodOpType = "Clear";
    break;
  case OP_TYPE_SCH_ADD:
    hr.smodOpType = "Add";
    break;
  case OP_TYPE_SCH_OVERWRITE:
    hr.smodOpType = "Overwrite";
    break;
  case OP_TYPE_SCH_REMOVE:
    hr.smodOpType = "Remove";
    break;
  case OP_TYPE_SCH_KEEP:
    hr.smodOpType = "Keep";
    break;
  default:
    hr.smodOpType = "    ?";
  ...
  uint8_t cmdOpType = writeBeBytesToLeNumber<uint32_t>(
      b + T_DIAG_CMD_MOD +
      T_MOD_INFO_TYPE ); // there may be more info here later, so don't use byte offsets, just mask (by cast now)
  // printf("Cmd Optype %02x @ 0x%08x, Flow would be %02x\n", cmdOpType, T_DIAG_CMD_MOD + T_MOD_INFO_TYPE,
  // OP_TYPE_CMD_FLOW);

  switch ( cmdOpType )
  {
  case OP_TYPE_CMD_FLOW:
    hr.cmodOpType = "Flow";
    break;
  case OP_TYPE_CMD_NOP:
    hr.cmodOpType = "No Op";
    break;
  case OP_TYPE_CMD_WAIT:
    hr.cmodOpType = "Wait";
    break;
  case OP_TYPE_CMD_FLUSH:
    hr.cmodOpType = "Flush";
    break;
  case OP_TYPE_CMD_START:
    hr.cmodOpType = "Start";
    break;
  case OP_TYPE_CMD_STOP:
    hr.cmodOpType = "Stop";
    break;
  case OP_TYPE_CMD_CEASE:
    hr.cmodOpType = "Cease";
    break;
  case OP_TYPE_CMD_ABORT:
    hr.cmodOpType = "Abort";
    break;
  default:
    hr.cmodOpType = "    ?";
...
void CarpeDM::CarpeDMimpl::show( const std::string& title,
                                 const std::string& logDictFile,
                                 TransferDir        dir,
                                 bool               filterMeta )
{
  ...
  BOOST_FOREACH ( vertex_t v, vertices( g ) )
  {
    maxLen = std::max( maxLen, g[v].name.length() );
  }

  sLog << std::endl
       << std::left << std::setfill( ' ' ) << std::setw( 3 ) << "Idx"
       << "   " << std::setfill( ' ' ) << std::setw( 3 ) << "S/R"
       << "   " << std::setfill( ' ' ) << std::setw( 3 ) << "Cpu"
       << "   " << std::setw( maxLen ) << "Name"
       << "   " << std::setw( 10 ) << "Hash"
       << "   " << std::setw( 10 ) << "Int. Adr"
       << "   " << std::setw( 10 ) << "Ext. Adr" << std::endl;
  ...
  BOOST_FOREACH ( vertex_t v, vertices( g ) )
  {
    ...
    {
      sLog << std::right << std::setfill( ' ' ) << std::setw( 4 ) << std::dec << v << "   " << std::setfill( ' ' )
           << std::setw( 2 ) << std::dec << (int)( at.isStaged( x ) ) << "   " << std::setfill( ' ' ) << std::setw( 3 )
           << std::dec << (int)x->cpu << "   " << std::setfill( ' ' ) << std::setw( maxLen ) << std::left << g[v].name
           << "   0x" << std::right << std::hex << std::setfill( '0' ) << std::setw( 8 ) << x->hash << "   0x"
           << std::right << std::hex << std::setfill( '0' ) << std::setw( 8 )
           << at.adrConv( AdrType::MGMT, AdrType::INT, x->cpu, x->adr ) << "   0x" << std::right << std::hex
           << std::setfill( '0' ) << std::setw( 8 ) << at.adrConv( AdrType::MGMT, AdrType::EXT, x->cpu, x->adr )
           << std::endl;
    }
  }
  ...
  sLog << std::endl
       << std::left << std::setw( maxLengthPatternName ) << std::setfill( ' ' ) << "Patterns"
       << std::setw( maxLengthEntryName ) << "Entry"
       << "Exit" << std::endl;

  for ( auto& it : gt.getAllPatterns() )
  {
    sLog << std::left << std::setw( maxLengthPatternName ) << std::setfill( ' ' ) << it
         << std::setw( maxLengthEntryName ) << getPatternEntryNode( it ) << getPatternExitNode( it ) << std::endl;
  }
  sLog << std::endl;
}
...
CarpeDM::CarpeDMimpl::coverage3TestData( uint64_t seedStart, uint64_t cases, uint8_t parts, uint8_t percentage )
{
  ...
  while ( ( tmp.size() < ( ( cases * percentage ) / 100 ) ) && ( seed < maxSeed ) )
  {
    if ( coverage3IsSeedValid( seed ) )
    {
      ...
      // std::cout << "rnd " << rnd << " thr " << (int)percentage << " " << ((rnd < (int)percentage) ? "YES" : "NO" ) <<
      // std::endl;
      if ( rnd < (int)percentage )
        tmp.push_back( seed );
    }
    seed++;
  }
  ...
  for ( unsigned i = 0; i < parts; i++ )
  {
    ...
    ret[i].insert( ret[i].end(),
                   tmp.begin() + thisOffs,
                   ( ( ( tmp.begin() + nextOffs ) <= tmp.end() ) ? ( tmp.begin() + nextOffs ) : tmp.end() ) );
  }
...
void CarpeDM::CarpeDMimpl::coverage3Upload( uint64_t seed )
{
  ...
  BOOST_FOREACH ( vertex_t v, vertices( g ) )
  {
    g[v].hash = hm.hash( g[v].name );
  } // generate hash to complete vertex information
  updateModTime();
  overwrite( g, true );
  download();
...
std::string CarpeDM::CarpeDMimpl::coverage3GenerateCursor( Graph& g, uint64_t seed )
{
  ...
  {
    cursor = ( curInit & 1 ) ? g[patExit[patName[curInit >> 1]]].name : g[patEntry[patName[curInit >> 1]]].name;
  }
