...
vEbwrs CarpeDM::CarpeDMimpl::gatherUploadVector( std::set<uint8_t> moddedCpus, uint32_t modCnt, uint8_t opType )
{
  ...
  for ( uint32_t memoryIndex = 0; memoryIndex < atUp.getMemories().size(); memoryIndex++ )
  {
    // generate addresses of Bmp's address range
    uint32_t startAddress =
        atUp.adrConv( AdrType::MGMT, AdrType::EXT, memoryIndex, atUp.getMemories()[memoryIndex].bmpOffs );
    uint32_t endAddress =
        atUp.adrConv( AdrType::MGMT, AdrType::EXT, memoryIndex, atUp.getMemories()[memoryIndex].startOffs );

    for ( uint32_t adr = startAddress; adr < endAddress; adr += _32b_SIZE_ )
    {
      ew.vcs.push_back( adr == startAddress );
      ew.va.push_back( adr );
    }
    // add Bmp to to return vector
    ew.vb += atUp.getMemories()[memoryIndex].getBmp();
  }
  ...
  for ( auto& it : atUp.getTable().get<CpuAdr>() )
  {
    ...
    if ( it.staged )
    {
      ...
      for ( uint32_t adr = atUp.adrConv( AdrType::MGMT, AdrType::EXT, it.cpu, it.adr );
            adr < atUp.adrConv( AdrType::MGMT, AdrType::EXT, it.cpu, it.adr + _MEM_BLOCK_SIZE );
            adr += _32b_SIZE_ )
      {
  ...
  for ( auto& itMod : moddedCpus )
  {
    createSchedModInfo( ew, itMod, modCnt, opType );
  }
...
void CarpeDM::CarpeDMimpl::generateDstLst( Graph& g, vertex_t v, unsigned multiDst )
{
  ...
  for ( unsigned i = 0; i < loops; i++ )
  {
    ...
    log<DEBUG_LVL0>( L"generateDstLst: Accomodating %1% + 1 destinations. Loop %2%/%3%, generating %4% " ) % multiDst %
        i % loops % name.c_str();
    hm.add( name );
    vertex_t vD = boost::add_vertex(
        myVertex( name, g[v].cpu, hm.lookup( name ), nullptr, dnt::sDstList, DotStr::Misc::sHexZero ), g );
    ...
    log<DEBUG_LVL1>( L"generateDstLst: Adding Edge from %1% to %2%. Setting pattern name to %3%" ) %
        g[source( thisEdge, g )].name.c_str() % g[target( thisEdge, g )].name.c_str() % g[vD].patName.c_str();
  }
}
...
void CarpeDM::CarpeDMimpl::generateQmeta( Graph& g, vertex_t v, int prio )
{
  ...
  vertex_t vBl = boost::add_vertex(
      myVertex( nameBl, g[v].cpu, hm.lookup( nameBl ), nullptr, dnt::sQInfo, std::string( DotStr::Misc::sHexZero ) ),
      g );
  vertex_t vB0 = boost::add_vertex(
      myVertex( nameB0, g[v].cpu, hm.lookup( nameB0 ), nullptr, dnt::sQBuf, DotStr::Misc::sHexZero ), g );
  vertex_t vB1 = boost::add_vertex(
      myVertex( nameB1, g[v].cpu, hm.lookup( nameB1 ), nullptr, dnt::sQBuf, DotStr::Misc::sHexZero ), g );
...
void CarpeDM::CarpeDMimpl::generateBlockMeta( Graph& g, bool doGenerateDstLst )
{
  ...
  BOOST_FOREACH ( vertex_t v, vertices( g ) )
  {
    ...
    if ( ( cmp == dnt::sBlockFixed ) || ( cmp == dnt::sBlockAlign ) || ( cmp == dnt::sBlock ) )
    {
      ...
      {
        if ( g[*out_cur].type == det::sQPrio[PRIO_IL] )
          hasIl = true;
        if ( g[*out_cur].type == det::sQPrio[PRIO_HI] )
          hasHi = true;
        if ( g[*out_cur].type == det::sQPrio[PRIO_LO] )
          hasLo = true;
      ...
      log<DEBUG_LVL1>( L"generateBlockMeta: Checking Block %1%. MultiDst=%2% hasDstLst=%3%" ) % g[v].name.c_str() %
          multiDst % hasDstLst;
...
void CarpeDM::CarpeDMimpl::updateStaging( vertex_t v, edge_t e )
{
  ...
  */
  if ( g[e].type != det::sAltDst )
  {
    ...
    // If we'd also change def dst, doing it before DM does has no effect, and doing it after the DM did would overwrite
    // the safe def dst
    if ( !isCovenantPending( g[x->v].name ) )
      at.setStaged( x );
    else
    {
      log<INFO>( L"updateStaging: Node %1% has an active covenant. Skipping staging to avoid race condition." ) %
          g[v].name.c_str();
    }
  }
}
...
void CarpeDM::CarpeDMimpl::prepareUpload()
{
  ...
  BOOST_FOREACH ( vertex_t v, vertices( gUp ) )
  {
    ...
    // try{
    // if (!(hm.lookup(name)))                   {throw std::runtime_error("Node '" + name + "' was unknown to the
    // hashmap"); return;}
    hash = gUp[v].hash;
    try
    {
    ...
    amI it =
        atUp.lookupHashNoEx( hash ); // if we already have a download entry, keep allocation, but update vertex index
    if ( !atUp.isOk( it ) )
    {
      // sLog << "Adding " << name << std::endl;
      allocState = atUp.allocate( cpu, hash, v, gUp, true );
      if ( allocState == ALLOC_NO_SPACE )
      {
        throw std::runtime_error( "Not enough space in CPU " + std::to_string( cpu ) + " memory pool" );
        return;
      }
      if ( allocState == ALLOC_ENTRY_EXISTS )
      {
        throw std::runtime_error( "Node '" + name + "' would be duplicate in graph." );
        return;
      }
    ...
    // Ugly as hell. But otherwise the bloody iterator will only allow access to MY alloc buffers (not their pointers!)
    // as const!
    auto* x = (AllocMeta*)&( *it );
    ...
    try
    {
      ...
      if ( gUp[v].np == nullptr )
      {
        ...
        if ( cmp == dnt::sTMsg )
        {
          completeId( v, gUp ); // create ID from SubId fields or vice versa
          gUp[v].np = ( node_ptr ) new TimingMsg( gUp[v].name,
                                                  gUp[v].patName,
                                                  gUp[v].bpName,
                                                  x->hash,
                                                  x->cpu,
                                                  flags,
                                                  s2u<uint64_t>( gUp[v].tOffs ),
                                                  s2u<uint64_t>( gUp[v].id ),
                                                  s2u<uint64_t>( gUp[v].par ),
                                                  s2u<uint32_t>( gUp[v].tef ),
                                                  s2u<uint32_t>( gUp[v].res ) );
        }
        else if ( cmp == dnt::sCmdNoop )
        {
          gUp[v].np = ( node_ptr ) new Noop( gUp[v].name,
                                             gUp[v].patName,
                                             gUp[v].bpName,
                                             x->hash,
                                             x->cpu,
                                             flags,
                                             s2u<uint64_t>( gUp[v].tOffs ),
                                             s2u<uint64_t>( gUp[v].tValid ),
                                             s2u<uint8_t>( gUp[v].prio ),
                                             s2u<uint32_t>( gUp[v].qty ),
                                             s2u<bool>( gUp[v].vabs ) );
        }
        else if ( cmp == dnt::sCmdFlow )
        {
          gUp[v].np = ( node_ptr ) new Flow( gUp[v].name,
                                             gUp[v].patName,
                                             gUp[v].bpName,
                                             x->hash,
                                             x->cpu,
                                             ...
                                             s2u<uint32_t>( gUp[v].qty ),
                                             s2u<bool>( gUp[v].vabs ),
                                             s2u<bool>( gUp[v].perma ) );
        }
        else if ( cmp == dnt::sSwitch )
        {
          gUp[v].np = ( node_ptr ) new Switch(
              gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags, s2u<uint64_t>( gUp[v].tOffs ) );
        }
        else if ( cmp == dnt::sOrigin )
        {
          gUp[v].np = ( node_ptr ) new Origin( gUp[v].name,
                                               gUp[v].patName,
                                               gUp[v].bpName,
                                               x->hash,
                                               x->cpu,
                                               flags,
                                               s2u<uint64_t>( gUp[v].tOffs ),
                                               s2u<uint8_t>( gUp[v].thread ) );
        }
        else if ( cmp == dnt::sStartThread )
        {
          gUp[v].np = ( node_ptr ) new StartThread( gUp[v].name,
                                                    gUp[v].patName,
                                                    gUp[v].bpName,
                                                    x->hash,
                                                    x->cpu,
                                                    flags,
                                                    s2u<uint64_t>( gUp[v].tOffs ),
                                                    s2u<uint64_t>( gUp[v].startOffs ),
                                                    s2u<uint32_t>( gUp[v].thread ) );
        }
        else if ( cmp == dnt::sCmdFlush )
        {
          gUp[v].np = ( node_ptr ) new Flush( gUp[v].name,
                                              gUp[v].patName,
                                              gUp[v].bpName,
                                              x->hash,
                                              x->cpu,
                                              ...
                                              s2u<bool>( gUp[v].vabs ),
                                              s2u<bool>( gUp[v].perma ),
                                              s2u<uint8_t>( gUp[v].frmIl ),
                                              s2u<uint8_t>( gUp[v].toIl ),
                                              s2u<uint8_t>( gUp[v].frmHi ),
                                              s2u<uint8_t>( gUp[v].toHi ),
                                              s2u<uint8_t>( gUp[v].frmLo ),
                                              s2u<uint8_t>( gUp[v].toLo ) );
        }
        else if ( cmp == dnt::sCmdWait )
        {
          gUp[v].np = ( node_ptr ) new Wait( gUp[v].name,
                                             gUp[v].patName,
                                             gUp[v].bpName,
                                             x->hash,
                                             x->cpu,
                                             ...
                                             s2u<uint64_t>( gUp[v].tValid ),
                                             s2u<uint8_t>( gUp[v].prio ),
                                             s2u<uint64_t>( gUp[v].tWait ),
                                             s2u<bool>( gUp[v].vabs ) );
        }
        else if ( cmp == dnt::sBlock )
        {
          gUp[v].np = ( node_ptr ) new BlockFixed(
              gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags, s2u<uint64_t>( gUp[v].tPeriod ) );
        }
        ...
        {
          gUp[v].np = ( node_ptr ) new BlockAlign(
              gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags, s2u<uint64_t>( gUp[v].tPeriod ) );
        }
        else if ( cmp == dnt::sQInfo )
        {
          gUp[v].np = ( node_ptr ) new CmdQMeta( gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags );
        }
        else if ( cmp == dnt::sDstList )
        {
          gUp[v].np = ( node_ptr ) new DestList( gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags );
        }
        else if ( cmp == dnt::sQBuf )
        {
          gUp[v].np = ( node_ptr ) new CmdQBuffer( gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags );
        }
        else if ( cmp == dnt::sGlobal )
        {
          gUp[v].np = ( node_ptr ) new Global(
              gUp[v].name, gUp[v].patName, gUp[v].bpName, x->hash, x->cpu, flags, gUp[v].section );
        }
  ...
  BOOST_FOREACH ( vertex_t v, vertices( gUp ) )
  {
    ...
    if ( debug || foundUninitialised )
    {
      log<DEBUG_LVL1>( L"prepareUpload: @ %1$#08x \n %2%" ) %
          atUp.adrConv( AdrType::MGMT, AdrType::INT, x->cpu, x->adr ) %
          hexDump( gUp[v].name.c_str(), haystack.c_str(), _MEM_BLOCK_SIZE ).c_str();
    }
...
{
  updateModTime();
  // we only regard modifications by order as modded, so we need to check for changed content before we generate the
  // management data
  std::set<uint8_t> moddedCpus;
  for ( unsigned int i = 0; i < atUp.getMemories().size(); i++ )
  {
    if ( !freshDownload || ( atUp.getMemories()[i].getBmp() != atDown.getMemories()[i].getBmp() ) )
      moddedCpus.insert( i ); // mark cpu as modified if alloctable empty or Bmp Changed
  }
   ...
   /
    ...
    while (adri != ewOrphans.va.end() and dati != ewOrphans.vb.end())
    {
      ...
      if(iHit != ewChg.va.end()) {
        ...
        auxstream << " A 0x" << std::setfill('0') << std::setw(8) << std::hex << *adr << " D 0x" << std::setfill('0') <<
   std::setw(8) << std::hex << val << " Destroys: 0x" << std::setfill('0') << std::setw(8) << std::hex << hitval <<
   std::endl; sDebug += auxstream.str();
      }
      adri++;
      dati+=4;
    }
    if (sDebug.size()  > 0) {
      sErr << "Possible access violation: Orphaned command cleanup routine tried to overwrite otherwise modified nodes.
   List of conflicting EB write ops:\n" << sDebug << std::endl;
      //throw std::runtime_error("Possible access violation: Orphaned command cleanup routine tried to overwrite
   otherwise modified nodes. List of conflicting EB write ops:\n" + sDebug);
...
void CarpeDM::CarpeDMimpl::addition( Graph& gTmp )
{
  ...
  BOOST_FOREACH ( vertex_t w, vertices( gTmp ) )
  {
    ...
    if ( atUp.isOk( x ) )
    {
      // Check how the duplicate is defined. Implicit (by edge) is okay, explicit is not. necessary to avoid
      // unintentional merge of graph nodes of the same name Check the type: if it's undefined, node definition was
      // implicit
      if ( gTmp[w].type != sUndefined )
        throw std::runtime_error(
            "Node " + gTmp[w].name +
            " already exists. You can only use the name again in an edge descriptor (implicit definition)" );
      duplicates[x->v] = w;
    }
  }
  ...
  // for(auto& it : vertexMap ) {sLog <<  "gTmp " << gTmp[it.first].name << " @ " << it.first << " gUp " << it.second <<
  // std::endl; } merge duplicate nodes
  ...
  for ( auto& itDup : duplicates )
  {
    ...
    // remove_vertex() changes the vertex vector, as it must stay contignuous. descriptors higher than he removed one
    // therefore need to be decremented by 1
    for ( auto& updateMapIt : vertexMap )
    {
      if ( updateMapIt.first > itDup.second )
        updateMapIt.second--;
    }
  }
  ...
  BOOST_FOREACH ( vertex_t v, vertices( gUp ) )
  {
    ...
    {
      throw std::runtime_error( "Parser error when processing entry/exit tags of node <" + gUp[v].name +
                                ">. Cause: " + err.what() );
    }
  }
...
void CarpeDM::CarpeDMimpl::pushMetaNeighbours( vertex_t v, Graph& g, vertex_set_t& s )
{
  ...
  BOOST_FOREACH ( vertex_t w, adjacent_vertices( v, g ) )
  {
    ...
    if ( g[w].np == nullptr )
    {
      throw std::runtime_error( "Node " + g[w].name + " does not have a data object, this is bad" );
    }
    if ( g[w].np->isMeta() )
    {
      s.insert( w );
...
void CarpeDM::CarpeDMimpl::subtraction( Graph& gTmp )
{
  ...
  // TODO probably a more elegant solution out there, but I don't have the time for trial and error on boost property
  // maps. create 1:1 vertex map for all vertices in gUp initially marked for deletion. Also add all their meta children
  ...
  BOOST_FOREACH ( vertex_t w, vertices( gTmp ) )
  {
    ...
    {
      toDelete.insert( x->v );
      log<DEBUG_LVL0>( L"subtraction: Checking adjacent vertices of node %1%" ) %
          gUp[x->v].name.c_str();                // add the node
  ...
  for ( auto& vd : toDelete )
  {
    ...
    atUp.deallocate(
        gUp[vertexMap[vd]].hash ); // using the hash is independent of vertex descriptors, so no remapping necessary yet
    ...
    // FIXME this removal scheme is crap ! Graph vertices and edges ought to be kept in setS or listS so iterators stay
    // valid on removal of other vertices However, the cure is worse than the disease. graphviz_write, copy_graph all
    // fuck around if we do, as they require vertex descriptors wich setS and listS don't provide ... leave for now

    // remove_vertex() changes the vertex vector, as it must stay contignuous. descriptors higher than he removed one
    // therefore need to be decremented by 1
    for ( auto& updateMapIt : vertexMap )
    {
      if ( updateMapIt.first > vd )
        updateMapIt.second--;
    }
  }
  ...
  for ( amI it = atUp.getTable().begin(); it != atUp.getTable().end(); it++ )
  {
    itAtVec.push_back( it );
  }

  for ( auto itIt : itAtVec )
  { // now we can safely iterate over the alloctable iterators
...
void CarpeDM::CarpeDMimpl::checkTablesForSubgraph( Graph& g )
{
  ...
  {
    // Check Hashtable
    if ( !hm.lookup( g[v].name ) )
    {
      throw std::runtime_error( "Node <" + g[v].name +
                                "> was explicitly named for keep/remove, but is unknown to Hashtable!\n" );
    }
    // Check Groupstable
    ...
    {
      throw std::runtime_error( "Node <" + g[v].name +
                                "> was explicitly named for keep/remove, but is unknown to Grouptable!\n" );
    }
  }
}
...
int CarpeDM::CarpeDMimpl::add( Graph& g, bool force )
{
  ...
  if ( ( boost::get_property( g, boost::graph_name ) ).find( DotStr::Graph::Special::sCmd ) != std::string::npos )
  {
    throw std::runtime_error( "Expected a schedule, but these appear to be commands (Tag '" +
                              DotStr::Graph::Special::sCmd + "' found in graphname)" );
    return -1;
  }
  if ( verbose )
    sLog << "Download binary as base for addition" << std::endl;
  baseUploadOnDownload();
  if ( verbose )
    sLog << "Add new subgraph" << std::endl;
  addition( g );
  // writeUpDotFile("upload.dot", false);
  validate( gUp, atUp, force );
  if ( verbose )
    sLog << "Upload" << std::endl;
...
int CarpeDM::CarpeDMimpl::remove( Graph& g, bool force )
{
  ...
  // FIXME fuckin dangerous stuff, both change the global grouptable. For this to work, it must be 1st
  // baseUploadOnDownload, generateBlockMeta must be 2nd. This is horrible
  baseUploadOnDownload();
  // generateBlockMeta(g);
  ...
  if ( !( force | ( isSafe ) ) )
  {
    throw std::runtime_error( "//Subgraph cannot safely be removed!\n\n" + report );
  }
...
int CarpeDM::CarpeDMimpl::keep( Graph& g, bool force )
{
  ...
  BOOST_FOREACH ( vertex_t w, vertices( gUp ) )
  {
    ...
    if ( !found )
    {
      boost::add_vertex( myVertex( gUp[w] ), gTmpRemove );
    }
  }
...
{
  nullify(); // read out current time for upload mod time (seconds, but probably better to use same format as DM FW.
             // Convert to ns)
  ...
  if ( !force && activity )
  {
    throw std::runtime_error( "Cannot clear, threads are still running. Call stop/abort/halt first\n" );
  }
...
int CarpeDM::CarpeDMimpl::overwrite( Graph& g, bool force )
{
  ...
  if ( !force && activity )
  {
    throw std::runtime_error( "Cannot overwrite, threads are still running. Call stop/abort/halt first\n" );
  }
