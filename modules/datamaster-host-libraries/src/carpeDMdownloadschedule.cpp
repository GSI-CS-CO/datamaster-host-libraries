...
// Generate download Bmp addresses. For downloads, this has to be two pass: get bmps first, then use them to get the
// node locations to read
vEbrds CarpeDM::CarpeDMimpl::gatherDownloadBmpVector()
{
  ...
  for ( uint32_t i = 0; i < at.getMemories().size(); i++ )
  {
    // generate addresses of Bmp's address range
    uint32_t startAdr = at.adrConv( AdrType::MGMT, AdrType::EXT, i, at.getMemories()[i].bmpOffs );
    uint32_t endAdr   = at.adrConv( AdrType::MGMT, AdrType::EXT, i, at.getMemories()[i].startOffs );

    for ( uint32_t adr = startAdr; adr < endAdr; adr += _32b_SIZE_ )
    {
      er.va.push_back( adr );
      er.vcs.push_back( adr == startAdr );
    }
  }
  return er;
}
...
vEbrds CarpeDM::CarpeDMimpl::gatherDownloadDataVector()
{
  ...
  {
    // go through a memory's bmp bits, starting at number of nodes the bmp itself needs (bmpSize / memblocksize).
    // Otherwise, we'd needlessly download the bmp again

    // sLog << "Bmp" << i << ":" << std::endl;
    for ( unsigned int bitIdx = at.getMemories()[i].bmpSize / _MEM_BLOCK_SIZE; bitIdx < at.getMemories()[i].bmpBits;
          bitIdx++ )
    {
      // if the bit says the node is used, we add the node to read addresses
      // sLog << "Bit Idx " << bitIdx << " valid " << at.getMemories()[i].getBmpBit(bitIdx) << " na 0x" << std::hex <<
      // at.getMemories()[i].sharedOffs + bitIdx * _MEM_BLOCK_SIZE << std::endl;
      if ( at.getMemories()[i].getBmpBit( bitIdx ) )
      {
        ...
        for ( uint32_t adr = at.adrConv( AdrType::MGMT, AdrType::EXT, i, nodeAdr );
              adr < at.adrConv( AdrType::MGMT, AdrType::EXT, i, nodeAdr + _MEM_BLOCK_SIZE );
              adr += _32b_SIZE_ )
        {
          er.va.push_back( adr );
...
void CarpeDM::CarpeDMimpl::parseDownloadMgmt( const vBuf& downloadData )
{
  ...
  for ( unsigned int i = 0; i < at.getMemories().size(); i++ )
  {
          ...
          bitIdx++ )
    {
      if ( at.getMemories()[i].getBmpBit( bitIdx ) )
      {
        ...
        if ( type != NODE_TYPE_MGMT )
          continue; // skip all non mgmt nodes
        uint8_t  cpu = i;
        ...
        if ( !( at.insertMgmt( cpu, adr, (uint8_t*)&downloadData[localAdr] ) ) )
        {
          throw std::runtime_error( std::string( "Address collision when adding mgmt node at " ) );
        };
        found++;
        // at.getMemories()[i].clrBmpBit(bitIdx); // clear bit so parseDownloadData doesnt have to deal with this node
        // again. IMPORTANT: this saves a little bit of work, but also means the bmp is out of sync until we leave this
        // function
      }
    }
  }
  ...
  if ( verbose )
    sLog << "Mgmt found " << std::dec << found << " data chunks. Total " << nodeCnt
         << " nodes scanned. Trying to recover GroupTable ..." << std::endl;
  ...
  if ( verbose )
    sLog << "Bytes expected: " << std::dec << at.getMgmtTotalSize() << ", recovered: " << std::dec << aux.size()
         << std::endl
         << std::endl;
  if ( tmpMgmtRecovery.size() )
  {
    ...
    if ( tmpStrGrouptab.size() )
    {
      gtTmp.load( tmpStrGrouptab );
      gt = gtTmp;
    }
    ...
    if ( tmpStrCovtab.size() )
    {
      ctTmp.load( tmpStrCovtab );
      ct = ctTmp;
    }
    ...
    if ( tmpStrReftab.size() )
    {
      rtTmp.load( tmpStrReftab );
      rt = rtTmp;
    }
  ...
  else
  {
    if ( verbose )
      sLog
          << "Management recovery returned empty, this happens when accessing a virgin DM FW memory. Skip Grouptable and Covenant Table creation"
          << std::endl;
  }
...
void CarpeDM::CarpeDMimpl::parseDownloadData( const vBuf& downloadData )
{
  ...
  // sLog << std::dec << "dl size " << downloadData.size() << std::endl;
  if ( verbose )
    sLog << "Analysing downloaded graph binary " << std::dec << downloadData.size() << " bytes" << std::endl;
  uint32_t nodeCnt = 0;
  ...
  for ( unsigned int i = 0; i < at.getMemories().size(); i++ )
  {
          ...
          bitIdx++ )
    {
      if ( at.getMemories()[i].getBmpBit( bitIdx ) )
      {
        ...
        uint32_t flags = writeBeBytesToLeNumber<uint32_t>(
            (uint8_t*)&downloadData[localAdr + NODE_FLAGS] ); // FIXME what about future requests to hashmap if we
                                                              // improvised the name from hash? those will fail ...
        uint32_t type = ( flags >> NFLG_TYPE_POS ) & NFLG_TYPE_MSK;
        uint8_t  cpu  = i;
        ...
        vertex_t v =
            boost::add_vertex( myVertex( name, pattern, beamproc, std::to_string( cpu ), hash, nullptr, "", tmp ), g );
        ...
        {
        case NODE_TYPE_TMSG:
          g[v].np   = ( node_ptr ) new TimingMsg( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sTMsg;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_CNOOP:
          g[v].np   = ( node_ptr ) new Noop( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sCmdNoop;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_CFLOW:
          g[v].np   = ( node_ptr ) new Flow( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sCmdFlow;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_CSWITCH:
          g[v].np   = ( node_ptr ) new Switch( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sSwitch;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_ORIGIN:
          g[v].np   = ( node_ptr ) new Origin( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sOrigin;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_STARTTHREAD:
          g[v].np   = ( node_ptr ) new StartThread( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sStartThread;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_CFLUSH:
          g[v].np   = ( node_ptr ) new Flush( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sCmdFlush;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_CWAIT:
          g[v].np   = ( node_ptr ) new Wait( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sCmdWait;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_BLOCK_FIXED:
          g[v].np   = ( node_ptr ) new BlockFixed( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sBlockFixed;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_BLOCK_ALIGN:
          g[v].np   = ( node_ptr ) new BlockAlign( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sBlockAlign;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_QUEUE:
          g[v].np   = ( node_ptr ) new CmdQMeta( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sQInfo;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_ALTDST:
          g[v].np   = ( node_ptr ) new DestList( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sDstList;
          g[v].np->deserialise( (uint8_t*)x->b );
          break;
        case NODE_TYPE_QBUF:
          g[v].np   = ( node_ptr ) new CmdQBuffer( g[v].name, g[v].patName, g[v].bpName, x->hash, x->cpu, flags );
          g[v].type = dnt::sQBuf;
          break;
        //   NODE_TYPE_GLOBAL can never occur here, similar to management nodes. Don't list it.
        case NODE_TYPE_UNKNOWN:
          sErr << "not yet implemented " << g[v].type << std::endl;
          break;
        default:
          sErr << "Node type 0x" << std::hex << type << " not supported! " << std::endl;
        }
      }
    }
  }
  ...
  if ( verbose )
    sLog << "Node creation done. Creating Edges" << std::endl;
  ...
  {
    // handled by visitor
    if ( g[it.v].np == nullptr )
    {
      throw std::runtime_error( std::string( "Node " ) + g[it.v].name + std::string( "not initialised" ) );
      return;
    }
    else
    {
  ...
  if ( verbose )
    sLog << "Done. Graph generation complete" << std::endl;
}
...
unsigned CarpeDM::CarpeDMimpl::recreateGlobalRefs()
{
  ...
  // we already recreated the reftable from the mamagement nodes we downloaded, Now, for each entry in rt, we create the
  // global node and insert into atDown.
  refIt rtBegin, rtEnd;
  ...
  for ( auto it = rtBegin; it != rtEnd; it++ )
  {
    ...
    if ( adrType != AdrType::INT )
    {
      throw std::runtime_error(
          std::string( "Error. GlobalReftable entry contains an address not part of the AdrType::INT" ) );
    }
    ...
    */
    ...
    // This is the same as for the normal nodes, but since globals are not part of the occupation bitmaps, we need to do
    // it here
    std::stringstream stream;
    stream.str( "" );
    stream.clear();
    ...
    // sLog << "Found the following from hm and gt: name " << name << " pattern " << pattern << " beamprocs " <<
    // beamproc << std::endl;

    // create node
    vertex_t v = boost::add_vertex(
        myVertex( name, pattern, beamproc, std::to_string( cpu ), hash, nullptr, "", DotStr::Misc::sZero ), g );
...
int CarpeDM::CarpeDMimpl::download()
{
  ...
  */
  ...
  if ( verbose )
    sLog << "Done." << std::endl << "Calling parser for Mgmt Meta" << std::endl;
  readMgmtLLMeta(); // we have to do this before parsing

  if ( verbose )
    sLog << "returned." << std::endl << "Calling parser for Mgmt Data" << std::endl;
  parseDownloadMgmt( vDlD );
  if ( verbose )
    sLog << "returned." << std::endl << "Calling parser for Download Meta" << std::endl;

  recreateGlobalRefs();

  parseDownloadData( vDlD );
  if ( verbose )
    sLog << "returned." << std::endl;
